            ...
            .map(({ link, label }, index) => (
              <React.Fragment key={label}>
                <li>
                  <PrismicNextLink
                    className={clsx(
                      "group relative block overflow-hidden  rounded px-3 py-1 text-base font-bold text-slate-600 transition-colors duration-150 hover:hover:text-yellow-400",
                    )}
                    field={link}
                  >
                    {label}
        ...
        <div className="socials inline-flex justify-center sm:justify-end">
          ...
          {isFilled.link(settings.data.linkedin) && (
            <PrismicNextLink
              ...
              className="p-2 text-2xl text-slate-900 transition-all duration-150 hover:scale-125 hover:text-yellow-400"
